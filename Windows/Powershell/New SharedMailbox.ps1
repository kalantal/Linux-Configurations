...
Function Main {
	...
	#Get user inputs
	Write-Host "This script will create a new shared mailbox on Office 365" -foregroundcolor green
	""
	Write-Host "Enter your Office 365 Administrator Username and Password"
	$cred = Get-Credential
	$SharedMailboxUserName = Read-Host "Enter a username for the new Shared mailbox (e.g. shared@jdcweb.com)" -foregroundcolor red
	$SharedMailboxDisplayName = Read-Host "Enter a friendly name for the new Shared mailbox (e.g. My Shared Mailbox)" -foregroundcolor red
	...
	""
	Write-Host "Waiting for 10 seconds for the mailbox to be created..." -foregroundcolor green
	""
	Start-Sleep -s 10
	""
	Write-Host "Setting 5GB Quota on Mailbox..." -foregroundcolor green
	""
	...
	while ($result -eq 0) 
	{
		...
		$UserToPermission = Read-Host "Enter the username for the person or group you want to give access to this mailbox (e.g you@jdcweb.com)"
